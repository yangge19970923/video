...
export default {
  ...
  activated: function() {
    ...
    this.isLoading = true
    this.getNowPlayingData(cityId)
  },
  methods: {
    getNowPlayingData: async function(cityId) {
      var { data: res } = await this.axios.get(
        '/api/movieOnInfoList?cityId=' + cityId
      )
      console.log(res.data.movieList[0])

      // console.log('/api/movieOnInfoList?cityId=' + cityId)
      if (res.msg === 'ok') {
        this.movieList = res.data.movieList
        this.isLoading = false
        this.prevCityId = cityId
        //this.$nextTick()可以保证页面渲染完毕
        // this.$nextTick(() => {
        //保证页面滑动的效果
    ...
    handleToDetail: function(movieId) {
